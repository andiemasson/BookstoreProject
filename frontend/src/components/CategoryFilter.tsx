...
function CategoryFilter ({
    ...
    useEffect(()=> {
        const fetchCategories = async () => {
            try {
                const response = await fetch('https://bookstore-clegg-backend.azurewebsites.net/Book/GetBookTypes');
            ...
            <div className='category-list'>
                {categories.map((c) => (
                    <div key={c} className="category-item">
                        <input type="checkbox" id={c} value={c} className="category-checkbox" onChange={handleCheckbox}/>
